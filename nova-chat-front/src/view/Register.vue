      ...
      <div class="right-section">
        ...
        <div class="form-content">
          ...
          <div class="avatar-upload-container">
            <div class="avatar-preview" :style="{ backgroundImage: avatarPreview ? `url(${avatarPreview})` : 'none' }" @click="triggerAvatarUpload">
          ...
          <button type="button" @click="handleRegister" class="submit-btn">立 即 注 册</button>
...
const handleMobileBlur = () => {
  ...
  if (!registerForm.value.mobile) {
    mobileError.value = '请输入手机号'
  } else if (!mobileReg.test(registerForm.value.mobile)) {
    mobileError.value = '请输入正确的手机号'
  } else {
...
const handleVerificationCodeBlur = () => {
  ...
  if (!registerForm.value.verificationCode) {
    verificationCodeError.value = '请输入验证码'
  } else if (registerForm.value.verificationCode.length !== 4) {
    verificationCodeError.value = '验证码应为4位数字'
  } else {
    verificationCodeError.value = ''
  }
...
const handleRegister = async () => {
  ...
  // 检查是否有错误
  if (usernameError.value || passwordError.value || mobileError.value || verificationCodeError.value) {
    return
  }
